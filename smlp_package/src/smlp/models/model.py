...
from smlp.optimizers.sgd import SGD
...
from smlp.activations import relu, sigmoid, softmax
...
class Model:
    ...
    def fit(
        ...
        if validation_data is not None:
            ...
            x_train, x_val, y_train, y_val = self._validation_split(
    ...
    def _make_batch_str(
        self, batch_start_time, batch_end_time, n_batch, batch_max, logs: dict
    ):
    ...
    def _callback_on_epoch_end(
        self, callbacks: list[Callback], epoch: int, logs: dict = None
    ):
    ...
    def _init_params(self) -> None:
        for i, (l, l_next) in enumerate(zip(self.layer, self.layer[1:]), start=1):
            self.params["W" + str(i)] = l_next.weights_initializer.initialize(
                (l.layer_dim, l_next.layer_dim)
            )
            self.params["b" + str(i)] = zeros.initialize((1, l_next.layer_dim))

    def _layer_check(self) -> None:
        assert len(self.layer) > 1, "check layers."
        assert (
            self.layer[0].type == LAYER.INPUT
        ), "first layer should be an input layer."

        for layer in self.layer[1:-1]:
            assert (
                layer.type != LAYER.INPUT
            ), "hidden layer should not be an input layer."
    ...
    def _linear_backward(
        self, dZ: np.ndarray, cache: tuple[np.ndarray, np.ndarray, np.ndarray]
    ):
        """
        Args
            dZ: gradient of the cost w.r.t. linear output
    ...
    def _model_backward_and_update_params(
        ...
        for i, layer in reversed(list(enumerate(self.layer[1:], start=0))):
            cache = caches[i]
            dAL, dW_tmp, db_tmp = self._linear_activation_backward(
                dAL, cache, layer.activation
            )
